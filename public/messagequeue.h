...
public:
	...
	int Pop(T &block, unsigned int timeout = INFINITE)	//从消息队列弹出一个包，超时返回-1，否则返回队列剩余的元素数量
	{
		int nSize = 0;
		if (WAIT_OBJECT_0 == WaitForSingleObject(m_hIsEmpty, timeout))
		{
			//读取并删除
			EnterCriticalSection(&m_csLock);
			block = m_queue.front();
			...
			if (!m_queue.empty())
				SetEvent(m_hIsEmpty);
			nSize = (unsigned int)m_queue.size();
			LeaveCriticalSection(&m_csLock);
			return nSize;
		}
		return -1;
