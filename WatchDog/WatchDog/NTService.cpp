...
BOOL CNTService::ParseStandardArgs(int argc, char* argv[])
{
	...
	else if (_stricmp(argv[1], "-i") == 0)
	{
		...
		else
		{
			...
			if (Install(argc - 2, argc > 2 ? &argv[2] : NULL))
                printf("%s installed\n", m_szServiceName);
			else
	...
	else if (_stricmp(argv[1], "-o") == 0)
	{
		if (!StartService(m_szServiceName))
			REPORT(MODULE_NAME, T("StartService() failed\n"), RPT_ERROR);
		return TRUE;
	...
	else if (_stricmp(argv[1], "-c") == 0)
	{
		if (!StopService(m_szServiceName))
			REPORT(MODULE_NAME, T("StopService() failed\n"), RPT_ERROR);
		return TRUE;
	}
...
BOOL CNTService::StartService(std::string strServName)
{
	...
	if(hSCM == NULL)
	{
		REPORT(MODULE_NAME, T("OpenSCManager() failed with error code:%d\n", GetLastError()), RPT_ERROR);
		return FALSE;
	}
	...
	if(hService == NULL)
	{
		REPORT(MODULE_NAME, T("OpenService() failed with error code:%d\n", GetLastError()), RPT_ERROR);
		SAFE_CLOSESCM(hSCM);
		return FALSE;
	}
	...
	{
		REPORT(MODULE_NAME, T("StartService() failed with error code:%d\n", GetLastError()), RPT_ERROR);
...
BOOL CNTService::StopService(std::string strServName)
{
	...
	{
		REPORT(MODULE_NAME, T("ControlService() failed with error code:%d\n", GetLastError()), RPT_ERROR);
		SAFE_CLOSESCM(hService);
		SAFE_CLOSESCM(hSCM);
		return FALSE;
	}
...
BOOL CNTService::Install(int argc, char* argv[])
{
    // Open the Service Control Manager
    SC_HANDLE hSCM = ::OpenSCManager(NULL, // local machine
                                     NULL, // ServicesActive database
	...
	{
		pos += sprintf(&szTmp[pos], "%s", argv[i]);
		pos++;
	}
