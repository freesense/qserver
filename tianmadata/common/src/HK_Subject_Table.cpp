...
CHK_Subject_Table::CHK_Subject_Table()
{
	m_nCount16 = -1;
	m_nCount17 = -1;
	m_nCount18 = -1;
...
BOOL CHK_Subject_Table::Open(const char* pFileName, DWORD dwDesiredAccess, DWORD dwShareMode,
...
void CHK_Subject_Table::Close()
{
	CBaseFile::Close();
}

BOOL CHK_Subject_Table::Requery()
{
	m_nCount16 = g_database.GetTableRecordCount("HK_STK16_Fbsc_R");
	m_nCount17 = g_database.GetTableRecordCount("HK_STK17_FHind_R");
	m_nCount18 = g_database.GetTableRecordCount("HK_STK18_ASSET1_R");
	m_nCount19 = g_database.GetTableRecordCount("HK_STK19_ASSET2_R");
	m_nCount20 = g_database.GetTableRecordCount("HK_STK20_PROFIT1_R");
	m_nCount21 = g_database.GetTableRecordCount("HK_STK21_PROFIT2_R");
	m_nCount22 = g_database.GetTableRecordCount("HK_STK22_CASH1_R");
	m_nCount23 = g_database.GetTableRecordCount("HK_STK23_CASH2_R");
	...
	if (!GetDataFromTable("HK_STK16_Fbsc_R", STK16_FBSC))
		m_nCount16 = 0;
	if (!GetDataFromTable("HK_STK17_FHind_R", STK17_FHIND))
		m_nCount17 = 0;
	if (!GetDataFromTable("HK_STK18_ASSET1_R", STK18_ASSET1))
		m_nCount18 = 0;
	if (!GetDataFromTable("HK_STK19_ASSET2_R", STK19_ASSET2))
		m_nCount19 = 0;
	if (!GetDataFromTable("HK_STK20_PROFIT1_R", STK20_PROFIT1))
		m_nCount20 = 0;
	if (!GetDataFromTable("HK_STK21_PROFIT2_R", STK21_PROFIT2))
		m_nCount21 = 0;
	if (!GetDataFromTable("HK_STK22_CASH1_R", STK22_CASH1))
		m_nCount22 = 0;
	if (!GetDataFromTable("HK_STK23_CASH2_R", STK23_CASH2))
		m_nCount23 = 0;

	return TRUE;
}
/*
void CHK_Subject_Table::LogEvent(LPCTSTR lpFormat, ...)
{
	char chMsg[200];
...
BOOL CHK_Subject_Table::WriteData()
{
	return TRUE;
}

BOOL CHK_Subject_Table::GetDataFromTable(LPCTSTR lpTableName, unsigned short nTableNum)
{
...
int CHK_Subject_Table::MovePos(unsigned short nTableNum)
{
	int nRecordCount = 0;
	CBaseFile::MoveFirst();
	switch(nTableNum)
