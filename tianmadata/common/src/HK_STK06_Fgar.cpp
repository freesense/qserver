...
BOOL CHK_STK06_Fgar::Open(const char* pFileName, DWORD dwDesiredAccess, DWORD dwShareMode,
...
void CHK_STK06_Fgar::Close()
{
	CBaseFile::Close();
}

BOOL CHK_STK06_Fgar::Requery()
{
	if (!g_database.OpenTable("HK_STK06_Fgar"))
	{
		REPORT(MN, T("OpenTable(\"HK_STK06_Fgar\") failed\n"), RPT_ERROR);
		return FALSE;
	}
...
/*
void CHK_STK06_Fgar::LogEvent(LPCTSTR lpFormat, ...)
{
	char chMsg[200];
...
BOOL CHK_STK06_Fgar::WriteData()
{
	return TRUE;
}

int CHK_STK06_Fgar::GetStockRecordCount(char* pStock)
{
	int nRecordCount = 0;
	CBaseFile::MoveFirst();
	tagPayOffCollectEx Info;
