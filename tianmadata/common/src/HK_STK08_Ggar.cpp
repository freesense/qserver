...
BOOL CHK_STK08_Ggar::Open(const char* pFileName, DWORD dwDesiredAccess, DWORD dwShareMode,
...
void CHK_STK08_Ggar::Close()
{
	CBaseFile::Close();
}

BOOL CHK_STK08_Ggar::Requery()
{
	if (!g_database.OpenTable("HK_STK08_Ggar"))
	{
		REPORT(MN, T("OpenTable(\"HK_STK08_Ggar\") failed\n"), RPT_ERROR);
		return FALSE;
	}
...
/*
void CHK_STK08_Ggar::LogEvent(LPCTSTR lpFormat, ...)
{
	char chMsg[200];
  ...
  */
BOOL CHK_STK08_Ggar::WriteData()
{
	return TRUE;
}

int CHK_STK08_Ggar::GetStockRecordCount(const char* pStock)
{
	int nRecordCount = 0;
	CBaseFile::MoveFirst();
	tagCommentaryCollectEx Info;
