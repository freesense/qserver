...
BOOL CCn_Stk01_Info::Open(const char* pFileName, DWORD dwDesiredAccess, DWORD dwShareMode,
...
void CCn_Stk01_Info::Close()
{
	CBaseFile::Close();
}

BOOL CCn_Stk01_Info::Requery()
{
	if (!g_database.OpenTable("CN_STK01_Info"))
	{
		REPORT(MN, T("OpenTable(\"CN_STK01_Info\") failed\n"), RPT_ERROR);
		return FALSE;
	}
...
/*
void CCn_Stk01_Info::LogEvent(LPCTSTR lpFormat, ...)
{
	char chMsg[200];
