...
BOOL CHK_MKT02_Qind::Open(const char* pFileName, DWORD dwDesiredAccess, DWORD dwShareMode,
...
void CHK_MKT02_Qind::Close()
{
	CBaseFile::Close();
}

BOOL CHK_MKT02_Qind::Requery()
{
	char szSql[400];
	sprintf_s(szSql, 400, "SELECT a.*, b.* FROM HK_MKT02_Qind AS a LEFT OUTER JOIN  HK_MKT03_Fnind AS b on a.idx_cd=b.idx_cd\
	...
	while(!g_database.IsEOF())
	{
		...
		g_database.GetCollect("hm0201", (LPCTSTR)index.cm0201, 10);
		pBufPos += 10;
		g_database.GetCollect("hm0202", (LPCTSTR)index.cm0202, 10);
		pBufPos += 10;
		g_database.GetCollect("hm0203", (LPCTSTR)index.cm0203, 10);
		pBufPos += 10;
		...
		for(int i=11; i<=25; i++)
		{
			...
			sprintf_s(szColumn, 10, "hm02%02d", i);
		...
		for(int i=1; i<=4; i++)
		{
			sprintf_s(szColumn, 10, "hm03%02d", i);
			g_database.GetCollect(szColumn, value);
			memcpy(pBufPos, &value, sizeof(double));
			pBufPos += sizeof(double);
		}
...
/*
void CHK_MKT02_Qind::LogEvent(LPCTSTR lpFormat, ...)
{
	char chMsg[200];
