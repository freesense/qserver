...
BOOL CCN_MKT01_Idx::Open(const char* pFileName, DWORD dwDesiredAccess, DWORD dwShareMode,
...
void CCN_MKT01_Idx::Close()
{
	FreeMemory();
	CBaseFile::Close();
}

BOOL CCN_MKT01_Idx::Requery()
{
	m_nDataSize = 0;
	FreeMemory();
	if (!g_database.OpenTable("CN_MKT01_Idx"))
	{
		REPORT(MN, T("OpenTable(\"CN_MKT01_Idx\") failed\n"), RPT_ERROR);
		return FALSE;
	}
	...
	while(!g_database.IsEOF())
	{
		...
		//mktIdx.pStockCode = g_database.GetMkt_Idx("CN_MKT04_Samp", (const char*)mktIdx.Idx_cd,
	...
	g_database.CloseTable();
	//获取指数样本股票
	if (!g_database.OpenTable("CN_MKT04_Samp"))
	{
		REPORT(MN, T("OpenTable(\"CN_MKT04_Samp\") failed\n"), RPT_ERROR);
		return FALSE;
	}
	//////////////////////////////////////////////////////////////
	CBaseFile::MoveFirst();
...
/*
void CCN_MKT01_Idx::LogEvent(LPCTSTR lpFormat, ...)
{
	char chMsg[200];
...
BOOL CCN_MKT01_Idx::WriteData()
{
	return TRUE;
}

void CCN_MKT01_Idx::FreeMemory()
{
	CBaseFile::MoveFirst();
	tagMkt_Idx mktIdx;
	while(!IsEOF())
