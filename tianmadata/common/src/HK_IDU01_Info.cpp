...
BOOL CHK_IDU01_Info::Open(const char* pFileName, DWORD dwDesiredAccess, DWORD dwShareMode,
...
void CHK_IDU01_Info::Close()
{
	CBaseFile::Close();
}

BOOL CHK_IDU01_Info::Requery()
{
	if (!g_database.OpenTable("HK_IDU01_Info"))
	{
		REPORT(MN, T("OpenTable(\"HK_IDU01_Info\") failed\n"), RPT_ERROR);
		return FALSE;
	}
...
/*
void CHK_IDU01_Info::LogEvent(LPCTSTR lpFormat, ...)
{
	char chMsg[200];
